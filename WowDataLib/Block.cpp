...
Block::Block(MCNK * mcnk):mcnk(mcnk)
{
	...
	for (float x=position.x;x<position.x+BLOCK_LENGTH+1;x+=CHUNK_LENGTH)
	{
		for (float y=position.y;y<position.y+BLOCK_LENGTH+1;y+=CHUNK_LENGTH) 
		{
			...
			vertices[count]=Vertice(Vector3(x,y,position.z+mcnk->mcvt.height[145-count-1]),Color(0.0f,1.0f,0.0f,1.0f));
