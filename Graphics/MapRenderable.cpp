...
void BlockRenderable::CreateEntity(Ogre::SceneNode * parent_scene)
{
	string parent_name=parent_scene->getName();
	string maual_name=parent_name+"_"+to_string(mcnk->coords.X)+"_"+to_string(mcnk->coords.Y);
...
void TileRenderable::CreateScene(Ogre::SceneNode * parent_scene)
{
	...
	for (int i=0;i<16;i++)
	{
		for (int j=0;j<16;j++)
		{
			((BlockRenderable*)blocks[i][j])->CreateEntity(scene_node);
		}
	}
}
